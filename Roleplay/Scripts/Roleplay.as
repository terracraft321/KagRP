...
shared class TDMSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			CInventory@ inv = playerBlob.getInventory();
			CBlob@ mat = server_CreateBlob("mat_wood");
			if (mat !is null)
			{
				mat.Tag("do not set materials");
				mat.server_SetQuantity(500);
				if (!playerBlob.server_PutInInventory(mat))
				{
				mat.setPosition(playerBlob.getPosition());
				}
			}
			CBlob@ mat2 = server_CreateBlob("mat_stone");
			if (mat2 !is null)
			{
				mat2.Tag("do not set materials");
				mat2.server_SetQuantity(250);
				if (!playerBlob.server_PutInInventory(mat2))
				{
				mat2.setPosition(playerBlob.getPosition());
				}
			}
		}
	}
	...
	Vec2f getSpawnLocation(PlayerInfo@ p_info)
	{
			...
			if (map !is null)
			{
				...
				f32 y = 0;
				int i = 0;
				//while (!map.isTileSolid(Vec2f(x,y)))
					while (i != 10){
						x = (XORRandom(map.tilemapwidth-65)+1) * map.tilesize + 32.0f;
						y = (XORRandom(map.tilemapheight-65)+1) * map.tilesize + 32.0f;
						if (!map.isTileSolid(Vec2f(x,y)))
							break;}
				//while (map.getLandYAtX(x) != 1000)
					//x = (XORRandom(map.tilemapwidth-65)+1) * map.tilesize + 32.0f;
				//f32 y = map.getLandYAtX(x);
				...
				f32 x = (XORRandom(map.tilemapwidth-65)+1) * map.tilesize + 32.0f;
				f32 y = 0;
				int posy = 0;
